...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $stmt = $pdo->prepare("SELECT `channel_name`, `admins`, `author_id` FROM `channels` WHERE `id`=?");
    writetolog($stmt, 'set');
    ...
    if ($res['admins'] != null) {
        $res['admins'] = json_decode($res['admins'], true);
        if (!in_array($my_id, $res['admins']) && $my_id != $res['author_id']) {
            $assent = true;
        }
        else {
            $assent = false;
    ...
    $messages_stmt->execute([':f'=>$my_id, ':s'=>'c-'.$whom_id, ':a'=>$whom_id, ':b'=>'c-'.$my_id]);
...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_POST['reply'] !== null) {
        $_SESSION['channel_data'] = $_POST['textarea'];
...
<body>
    ...
    <form action="<?=$_SERVER['PHP_SELF']?>" method='POST' enctype="multipart/form-data">
        ...
        <textarea name="textarea" <?if($assent == true):?>disabled<?endif;?>></textarea>
